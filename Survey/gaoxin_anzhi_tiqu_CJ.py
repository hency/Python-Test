...
def get_CW_value(name1):
    ...
    for i in range(0,nummz):
        if("桩顶竖向位移监测日报表" in cols[i] or "坡顶竖向位移监测日报表" in cols[i]):#边坡顶部
            numm=numm+1
    print(cols[nummz-10])
    for i in range(0,nummz):
        if ("桩顶竖向位移监测日报表" in cols[i] or "坡顶竖向位移监测日报表" in cols[i]):
            index1.append( i + 2 )
            ...
            for j in range(start_index_1,pd1.shape[0]):
                ...
                if 'CW' not in pd1.iloc[j, i]:
                    ...
                    end_index_1=j-1
                    break
            pd2.append(pd1.iloc[start_index_1:end_index_1+1,i+5])  ####本次高程选择i+3 而累计变化量的选择i+6
...
k=2
for j in range(len(subpath2_names)):   #len(subpath2_names)len(subpath2_names)len(subpath2_names)
    pd_concat=[]
    cw_concat=[]
    ...
    print(j)
book1.save(path+"\\"+'CW_高新安置_2019_速率.xlsx')
book1.close()
###############60存在问题
