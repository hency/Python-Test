...
dict1 = {num[i]: i for i in range(df4.shape[0])}
def sight_height_distance(df3,cx_df4,book1,sheet1,BC1):
    ...
    for i in range(cx_df4.shape[0]-1):
        ...
        if(dh>1.13):
            ...
            xx1=1/0
        height1[i]=cx_df4.iloc[i+1,1]-(i+1)*BC1
        sheet1.cell(7+5*i,9).value=height1
        #'高差值最大是1.8-0.55=1.25m' 且 dh_random的取值范围为0.55+dh~1.8
        dh1_random=dh+Sight_Height_random1[i]
        dh2=2*dh-dh1_random
        if (dh1_random > 0):
            RH1_random[i] = round(random.uniform(0.6 + dh1_random,1.75),5)  ###为了保证RH2_random的取值范围在0.55~1.8之间这里把目标范围设置为0.6~1.75之间
            FH1[i] = RH1_random - dh1_random
            RH2_random[i] = RH1_random + Sight_Height_random1[i]
            FH2[i] = RH2_random - dh2
            if ((abs(RH1_random - RH2_random) < 0.0003 and abs(FH1 - FH2) < 0.0003) and abs(dh1_random - dh2) < 0.0006):  # 进行判断基辅差与高差之差，由于仪器取位是5位，这里的随机数据选择五位
                print("输出")
            else:
            ...
            RH1_random[i] = round(random.uniform(0.6,1.75+dh1_random),5)  ###为了保证RH2_random的取值范围在0.55~1.8之间这里把目标范围设置为0.6~1.75之间
            FH1[i] = RH1_random -dh1_random
            RH2_random[i] = RH1_random + Sight_Height_random1[i]
            FH2[i] = RH2_random - dh2
            if ((abs(RH1_random - RH2_random) < 0.0003 and abs(FH1 - FH2) < 0.0003) and abs(dh1_random - dh2) < 0.0006):  # 进行判断基辅差与高差之差，由于仪器取位是5位，这里的随机数据选择五位
            ...
            RH1_random[i] = round(random.uniform(0.6,1.75),5)  ###为了保证RH2_random的取值范围在0.55~1.8之间这里把目标范围设置为0.6~1.75之间
            FH1[i] = RH1_random -dh1_random
            RH2_random[i] = RH1_random + Sight_Height_random1[i]
            FH2[i] = RH2_random - dh2
            if ((abs(RH1_random - RH2_random) < 0.0003 and abs(FH1 - FH2) < 0.0003) and abs(dh1_random - dh2) < 0.0006):  # 进行判断基辅差与高差之差，由于仪器取位是5位，这里的随机数据选择五位
    ...
    for i in range(cx_df4.shape[0]-1):
        ...
        HDb1[i]=HD+HD_random1
        HDb2[i]=HDb1+round(random.uniform(-0.005,0.005),3)
        HDf1[i]=HD+HD_random2
        HDf2[i]=HDf1+round(random.uniform(-0.005,0.005),3)
        HDb=(HDb1+HDb2)/2
        HDf=(HDf1+HDf2)/2
        HD_differ=HDb-HDf
        ...
        if(HD_differ_sum>6):
            ...
            print('前后视距累计差抄超限')
        k1=k1+5
    from ce1_2 import position_index
    ...
    for zz1 in range(len(position)):
        Z_name[zz1]=df3.loc[position[zz1][0],0]
        Z_HD[zz1]=df3.loc[position[zz1][0],2]
        Z_DH1=df3.loc[position[zz1][1],1]-df3.loc[position[zz1][0],1]
        Z_H[zz1]=df3.loc[position[zz1][1],1]-position[zz1][1]*BC1-Z_DH1
        Z_HB=RH1_random[dict1[position[zz1][1]]]
        Z_HF[zz1]=Z_HB+Z_DH1
    for zz1 in range(len(position)):
        sheet1.cell(7 + 5 * i, 9).value = height1
        if((i+1)%2==1):
