...
def get_CW_value(name1):
    ...
    for i in range(len(cols)):
        if("地下水位监测日报表" in cols[i]):
            nummz=i
            break
    for i in range(0,nummz):
        if("桩顶水平位移监测日报表" in cols[i] or "坡顶水平位移监测日报表" in cols[i]):#边坡顶部
            numm=numm+1
    print(cols[nummz-10])
    for i in range(0,nummz):
        if ("桩顶水平位移监测日报表" in cols[i] or "坡顶水平位移监测日报表" in cols[i]):
            index1.append( i + 2 )
            ...
            for j in range(start_index_1,pd1.shape[0]):
                ...
                if 'CW' not in pd1.iloc[j, i]:
                    ...
                    end_index_1=j-1
                    break
            pd2.append(pd1.iloc[start_index_1:end_index_1+1,i+6])
...
for j in range(1,77):
    sheet2.cell(1,j).value='CW'+str(j) ###根据实际的CW个数进行赋值这里是CW1-CW76
k=2
for j in range(len(subpath2_names)):   #len(subpath2_names)len(subpath2_names)
    ...
    for i in range(0, pd_concat.shape[0]):
        for z in range(1,77):
            if(sheet2.cell(1,z).value==cw_concat.iloc[i]):
                sheet2.cell(k,z).value=pd_concat.iloc[i]
                break
            else:
                pass
    k=k+1
    ...
    print(j)
book1.save(path+"\\"+'CW_高新安置_2019_WY_累计变化.xlsx')
book1.close()
###############60存在问题
