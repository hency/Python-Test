...
def output_excel():
    ...
    measure_line_path="D:\\Desktop\\主体数据库\\测线.xlsx"
    dateset_path="D:\\Desktop\\主体数据库\\2#数据库.xlsx"
    path_output1="D:\\Desktop\\主体数据库\\报表\\"
    qi_shu = 11
    BC_range1 = '-0.00015/0.00015'
    ...
    for m in range(int(qi_shu)):#len(RZ_values)
        BC1 = round(random.uniform(float(BC_range_min[0]), float(BC_range_max[0])), 5)
    ...
    if(pat_name[i][-4:]=='xlsx'):
        if(re.findall(r'数据(.*)',pat_name[i])[0]!='无支点.xlsx'):
            ...
            for j in range(3,nrows+1):
                sheet1.cell(j,3).value=datetime.timedelta(seconds=28800+(j-3)*15)   #print(datetime.timedelta(seconds=28800))
...
for z1 in range(len(pat_name)):
    ...
    if(pat_name[z1][-4:]=='xlsx'):
        if(re.findall(r'数据(.*)',pat_name[z1])[0]!='无支点.xlsx'):
            ...
            fid1.write('For M5|Adr'+'   '+'  1'+'|'+'TO'+'  '+date4+'.dat'+'               '+"|"+"                      "+"|"+"                      "+"|"+"                      "+"|"+"\n")
            fid1.write('For M5|Adr'+'   '+'  2'+'|'+'TO'+'  '+'Start-Line'+'      '+'aBFFB'+'     '+'1'+'|'+"                      "+"|"+"                      "+"|"+"                      "+"|"+"\n")
            rown=data1.shape[0]
            coln=data1.shape[1]
            i=0
            for i in range(0,rown):
                ...
                dmlength=kn+dm
                if(i==0):
                    fid1.write('For M5|Adr'+'   '+index1+'|'+'KD1'+dmlength+'                  '+'1'+'|'+"                      "+'|'+"                      "+'|'+'Z'+'        '+baoliu(str(data1.iloc[i,coln-1]))+' m'+'    '+'|'+'\n')
                else:
                    if( (~numpy.isnan(data1.iloc[i,4]))   and (~numpy.isnan(data1.iloc[i,coln-1])) ):
                        ##支点
                        if (len(baoliu3(str(data1.iloc[i, 6]))) == 6):
                            fid1.write('For M5|Adr' + '   ' + index1 + '|' + 'KD1' + dmlength + '      ' + str(
                                data1.iloc[i, 2]) + '    ' + '1' + '|' + data1.iloc[i, 5] + '        ' + baoliu5(
                                str(round(data1.iloc[i, 4],5))) + ' ' + 'm' + '   ' + '|' + 'HD' + '         ' + baoliu3(
                                str(round(data1.iloc[i, 6],3))) + ' m' + '   ' + '|' + 'Z'+'        '+baoliu5(str(round(data1.iloc[i,coln-1],5)))+' m'+'   '+'|'+'\n')
                        else:
                            fid1.write('For M5|Adr' + '   ' + index1 + '|' + 'KD1' + dmlength + '      ' + str(
                                data1.iloc[i, 2]) + '    ' + '1' + '|' + data1.iloc[i, 5] + '        ' + baoliu5(
                                str(round(data1.iloc[i, 4],5))) + ' ' + 'm' + '   ' + '|' + 'HD' + '          ' + baoliu3(
                        ...
                        ##后视点或者前视点
                        if(len(baoliu3(str(data1.iloc[i,6])))==6):
                            fid1.write('For M5|Adr' + '   ' + index1 + '|' + 'KD1' + dmlength+'      '+str(data1.iloc[i,2])+'    '+'1'+'|'+data1.iloc[i,5]+'        '+baoliu5(str(round(data1.iloc[i,4],5)))+' '+'m'+'   '+'|'+'HD'+'         '+baoliu3(str(round(data1.iloc[i,6],3)))+' m'+'   '+'|'+'                      '+'|'+'\n')
                        else:
                            fid1.write('For M5|Adr' + '   ' + index1 + '|' + 'KD1' + dmlength + '      ' + str(
                                data1.iloc[i, 2]) + '    ' + '1' + '|' + data1.iloc[i, 5] + '        ' + baoliu5(
                                str(round(data1.iloc[i, 4],5))) + ' ' + 'm' + '   ' + '|' + 'HD' + '          ' + baoliu3(
                                str(round(data1.iloc[i, 6],3))) + ' m' + '   ' + '|' + '                      ' + '|' + '\n')
                    else:
                        fid1.write('For M5|Adr' + '   ' + index1 + '|' + 'KD1' + dmlength + '      ' + str(
                            data1.iloc[i, 2]) + '    ' + '1' + '|' + '                      ' + '|' + '                      ' + '|' + 'Z' + '        ' + baoliu5(
            ...
            if(round(data1.iloc[i,8]-data1.iloc[0,8],5)<0):
                fid1.write('For M5|Adr'+ '   '+index3[0]+'|'+'KD1'+dmlength1+'                  '+'1'+'|'+'Sh'+'       '+baoliu5('%.5f'% (data1.iloc[i,8]-data1.iloc[0,8]))+' m'+'   '+'|'+'                      |                      |'+'\n')
            else:
                fid1.write('For M5|Adr' + '   ' + index3[
                    0] + '|' + 'KD1' + dmlength1 + '                  ' + '1' + '|' + 'Sh' + '        ' + baoliu5(
                    '%.5f'% (data1.iloc[i,8]-data1.iloc[0,8])) +' m'+ '   '+'|'+'                      |                      |'+'\n')
            ...
            if(len(re.findall(r'(.*)[.]',str(round(dbz,3)))[0])==3):
                fid1.write('For M5|Adr' + '   ' + index3[1] + '|' + 'KD1' + dmlength1+'      '+czs1+'        '+'1'+'|'+'Db'+'        '+baoliu3(str(round(dbz,3)))+' m'+'   '+'|'+'Df'+'        '+baoliu3(str(round(dfz,3))+' m'+'   '+'|'+ 'Z' + '        ' + baoliu(
                                str(data1.iloc[i, coln-1])) + ' m' + '   ' + '|' + '\n'))
            else:
                fid1.write('For M5|Adr' + '   ' + index3[1] + '|' + 'KD1' + dmlength1+'      '+czs1+'        '+'1'+'|'+'Db'+'        '+baoliu3(str(round(dbz,3)))+'  m'+'   '+'|'+'Df'+'        '+baoliu3(str(round(dfz,3))+'  m'+'   '+'|'+ 'Z' + '        ' + baoliu(
                                str(data1.iloc[i, coln-1])) + ' m' + '   ' + '|' + '\n'))
            fid1.write('For M5|Adr' + '   ' + index3[1] + '|' +'TO'+'  '+'End-Line'+'                  '+'1'+'|'+'                      |                      |                      |')
            fid1.close()
