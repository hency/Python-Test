...
class Zhuti_dataset(QDialog):
    ...
    def zhuti_dataset(self):
        ...
        for z in range(len(sheet_names)):
            ...
            for i in range(7, data1.shape[1], 7):
                if (re.findall(r'[(](.*)[)]', data1.iloc[1, i - 2], flags=0) == []):
                    sheet1.cell(data1.shape[0], k).value = re.findall(r'[（](.*)[）]', data1.iloc[1, i - 2], flags=0)[0]
                    sheet3.cell(12 + k, 2).value = re.findall(r'[（](.*)[）]', data1.iloc[1, i - 2], flags=0)[0]
                    sheet3.cell(12 + k, 1).value = k
                    sheet4.cell(1 + k, 2).value = re.findall(r'[（](.*)[）]', data1.iloc[1, i - 2], flags=0)[0]
...
class Jikeng_original_cj(QDialog):
    ...
    def jikeng_function_cj(self):
        ...
        for m in range(int(qi_shu)):  # len(RZ_values)
            ...
            if(flaggg%2==1):
                BC1 = round(random.uniform(-0.0008, -0.0004), 5)
            else:
                BC1 = round(random.uniform(0.0003, 0.0009), 5)
