...
class Create_zhuti_dir(QDialog):
    def __init__(self):
        ...
        self.label1=QLabel('第一步打开还原后的原始数据路径：原始数据命名需要按照规则命名',self)
        self.label2=QLabel('第二步打开所有栋楼数据库路径  ：数据库命名需要按照规则命名', self)
    ...
    def zhuti_dir(self):
        path1 = os.path.abspath(QFileDialog.getExistingDirectory(self,"选取还原后的原始数据路径",''))+'\\' # 起始路径"C:\\Users\\Kong\\Desktop\\2022\\主体沉降\\明园九龙湾主体\\沉降原始数据\\"
        pathx1 = os.path.abspath(QFileDialog.getExistingDirectory(self,"选取所有栋楼数据库路径",''))+'\\' # 起始路径"C:\\Users\\Kong\\Desktop\\2022\\主体沉降\\明园九龙湾主体\\数据库位置\\"
        filename = os.listdir(path1)
        num = []
        file_names = []
        for i in range(len(filename)):
            if(filename[i][-4:]=='.dat'):
                if (re.findall(r'(.*)#', filename[i], flags=0)[0] in num):
                    flag = 0
                else:
                    flag = 1
                    num.append(re.findall(r'(.*)#', filename[i], flags=0)[0])
                    k = 0
                    namex2 = []
                    for j in range(len(filename)):
                        if (re.findall(r'(.*)#', filename[i], flags=0)[0] == re.findall(r'(.*)#', filename[j], flags=0)[0]):
                            k = k + 1
                            namex2.append(filename[j])
                    file_names.append([re.findall(r'(.*)#', filename[i], flags=0)[0], k, namex2])
        if (os.path.isdir(os.path.abspath(
                os.path.join(path1, "..")) + '\\' + '原始数据报表')):
            print("原始数据报表已经存在")
        else:
        ...
        for i in range(len(file_names)):
            name_d = file_names[i][0]
            if (os.path.isdir(path2 + name_d + '#')):
                pass
            else:
                os.makedirs(path2 + name_d + '#')
            name_dataset_d_path = pathx1 + name_d + '#数据库'
            pd1 = pd.read_excel(name_dataset_d_path + '.xlsx', '日报')
            for j in range(int(file_names[i][1])):
                date1 = str(pd1.iloc[j, 1])
                ...
                datex2 = path2 + name_d + '#' + '\\' + x3
                os.makedirs(datex2)
                for z in range(len(file_names[i][2])):
                    if (re.findall(r'沉降(.*)[.]dat', file_names[i][2][z], flags=0)[0] == str(j + 1)):
                        shutil.copy(path1 + file_names[i][2][z], datex2 + '\\')
                        b_path.append(datex2+'\\'+file_names[i][2][z])
                        break
...
class Zhuti_bihecha(QDialog):
    ...
    def zhuti_bihecha(self):
        ...
        pathx1=os.path.abspath(os.path.join(path1, ".."))+'\\'
        filename=os.listdir(path1)
...
class Zhuti_dataset(QDialog):
    ...
    def zhuti_dataset(self):
        # path = "D:\\2022\\主体沉降\\中航城3期\\中航三期报告.xlsx"
        # path1 = "D:\\2022\\主体沉降\\中航城3期\\dataset.xlsx"
        #('D:/2022/主体沉降/中航城3期/中航三期报告.xlsx', 'Excel files(*.xlsx , *.xls)')
        #os.path.abspath(os.path.join(measure_line_path, "..")) + '\\'
        path = QFileDialog.getOpenFileName(self, '选择文件', '', 'Excel files(*.xlsx , *.xls)')[0]
        path1 = os.path.abspath(os.path.join(path, ".."))+'\\'+'dataset.xlsx' #############################################################
        path2= os.path.abspath(os.path.join(path, ".."))+'\\'+'数据库模板.xlsx'
...
class Zhuti_curv(QDialog):
    ...
    def zhuti_curv(self):
        path = QFileDialog.getOpenFileName(self, '选择文件', '', 'Excel files(*.xlsx , *.xls)')[0]
        path1 = os.path.abspath(os.path.join(path, ".."))+'\\'+'累计变化量.xlsx' #############################################################
        path2= os.path.abspath(os.path.join(path, ".."))+'\\'+'数据库模板.xlsx'
        book2 = load_workbook(path)
        sheet_names = book2.sheetnames
        book1 = openpyxl.Workbook()
        for z in range(len(sheet_names)):
            ...
            for i in range(index1, data1.shape[0]):
                if ('CJ' not in data1.loc[i, 0]):
                    ...
                    break
            for i in range(index1, index2):
                sheet1.cell(i - index1 + 1, 1).value = 0
                sheet1.cell(i - index1 + 1, 2).value = 0
            k = 3
            if (re.findall(r'[(](.*)[)]', data1.iloc[1, 0], flags=0) == []):
                sheet1.cell(data1.shape[0], 1).value = re.findall(r'[（](.*)[）]', data1.iloc[1, 0], flags=0)[0]
            else:
            ...
            for i in range(10, data1.shape[1], 7):
                if (re.findall(r'[(](.*)[)]', data1.iloc[1, i - 5], flags=0) == []):
                    sheet1.cell(data1.shape[0], k).value = re.findall(r'[（](.*)[）]', data1.iloc[1, i - 5], flags=0)[0]
                else:
                    sheet1.cell(data1.shape[0], k).value = re.findall(r'[(](.*)[)]', data1.iloc[1, i - 5], flags=0)[0]
                for j in range(index1, index2):
                    sheet1.cell(j - index1 + 1, k).value = data1.loc[j, i]
...
class Jikeng_original_cj(QDialog):
    ...
    def read_measure_line_from_dataset(self, dateset_path, measure_line_path):
        ...
        for i in range(0, dsheet3.max_row):
            if (dsheet3.cell(i + 1, 2).value == '' or dsheet3.cell(i + 1, 3).value is None):
                ...
                break
            elif (dsheet3.cell(dsheet3.max_row, 2).value != '' and dsheet3.cell(dsheet3.max_row,
                                                                                3).value is not None):
                max_row = dsheet3.max_row
        date = []
        max_col = 0
                                ...
                                for p in range(position2[q][1]):
                                    if (((abs(RH1_random_1 - RH2_random_1) < 0.0003 and abs(
                                        ...
                                        position2[q][2] - position2[q][1] + p] and Z_H1[
                                                position2[q][2] - position2[q][1] + p] < RH1_random_1 + df3.loc[
                                                dict1[
                                                    i], 1] - i * BC1 - 0.6)):  # 进行判断基辅差与高差之差，由于仪器取位是5位，这里的随机数据选择五位
                                        can_shu1 = can_shu1 + 1
        ...
        while (flag == 0):
            for zz1 in range(kx1, len(position)):
                ...
                for zz4 in range(zz3 - zz1):
                    ...
                    sheet2.cell(start_num + 5 * (zz2 - start_num_fei) + 5 + zz4, 5).value = Z_HF[
                        zz1 + zz4]  ##中间点视线高
    ...
    def jikeng_function_cj(self):
        ...
        # dateset_path = "D:\\2022\\基坑监测\\大剧院\\数据库\\保利大剧院数据库1.xlsx"
        # measure_line_path = "D:\\2022\\基坑监测\\大剧院\\数据库\\测线.xlsx"
        ...
        for m in range(start11-1,int(qi_shu)):  # len(RZ_values)
            ...
            if(flaggg%2==1):
                BC1 = round(random.uniform(-0.002, -0.0004), 5)
            else:
                BC1 = round(random.uniform(0.0006, 0.002), 5)
            ...
            self.sight_height_distance(df3, df4, book2, sheet2, BC2, path_output3, path_output2, num, dict1)
            pat1 = path_output1 + '\\'
            ...
            for i in range(len(pat_name)):
                if (pat_name[i][-4:] == 'xlsx'):
                    if (re.findall(r'数据(.*)', pat_name[i])[0] != '无支点.xlsx'):
                        ...
                        sheet2 = book2.get_sheet_by_name('周边道路沉降成果表') ############周边道路、周边管线、周边建筑、坡顶沉降成果表 @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            ...
            for z1 in range(len(pat_name)):
                ...
                if (pat_name[z1][-4:] == 'xlsx'):
                    if (re.findall(r'数据(.*)', pat_name[z1])[0] != '无支点.xlsx'):
                        ...
                        fid1 = open(path1 + '周边道路沉降' + INDEX3[num_1 - 1] + '.dat', 'w')  ##坡顶沉降、周边地表 ###########@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                            ...
                            if (i == 0):
                                ...
                                if ((~numpy.isnan(data1.iloc[i, 4])) and (~numpy.isnan(data1.iloc[i, coln - 1]))):
                                    ...
                                    if (len(self.baoliu3(str(data1.iloc[i, 6]))) == 6):
                                        fid1.write(
                                            'For M5|Adr' + '   ' + index1 + '|' + 'KD1' + dmlength + '      ' + str(
                                                data1.iloc[i, 2]) + '    ' + '1' + '|' + data1.iloc[
                                                i, 5] + '        ' + self.baoliu5(
        ...
        for i in range(len(file_names)):
            if ('沉降' in file_names[i]):  ##属于水准沉降原始文降
                if (f in file_names[i]):
                    ...
                    flagf = 1
        for i in range(len(file_names)):
            if (k in file_names[i]):
                qi_num = int(re.findall('第(.*)期', file_names[i], flags=0)[0])
                numk.append(qi_num)
                namek.append(file_names[i])
                flagk = 1
        for i in range(len(file_names)):
            if (m in file_names[i]):
                qi_num = int(re.findall('第(.*)期', file_names[i], flags=0)[0])
                numm.append(qi_num)
                namem.append(file_names[i])
                flagm = 1
        if (flaga == 0):
            numa = [0]
        if (flagb == 0):
